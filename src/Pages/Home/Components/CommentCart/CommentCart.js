...
export default function CommentCart(props) {
  const { _id, imgUrl, name, commentText, timestamp, userEmail, parentId } =
    props.comment;
  ...
  const deleteComment = (id, postId, postComments) => {
    if (window.confirm("Are you sure to delete this comment?")) {
      axios
        .delete(`http://localhost:5000/comment/${id}_${postId}_${postComments}`)
        .then((res) => {
          if (res) {
      ...
      <div className="flex items-start">
        ...
        <div className="w-full">
          ...
          <div className="w-1/12 ml-5 mt-2 flex justify-between">
            <button
              title="replay"
              className="text-textPrimary text-xs font-semibold mr-5"
            ...
            {userEmail === user.email && (
              <>
                ...
                <button
                  title="delete"
                  className="text-textPrimary text-xs font-semibold"
                  onClick={() => deleteComment(_id, postId, postComments)}
                >
                  Delete
                </button>
              </>
            ...
            <div className="relative mb-4 mr-1 w-full flex">
              ...
              {user.displayName ? (
                <>
                  ...
                  <label
                    htmlFor={parentId || "sub"}
                    className="cursor-pointer mt-5 outline-none flex items-center justify-center"
                  >
